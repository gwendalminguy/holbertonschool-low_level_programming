...
/**
 * wildcmp - compares two strings taking into account wildcards
 * @s1: first string
 * @s2: second strings
 *
...
/**
 * compare_strings - compares two strings taking into account wildcards
 * @s1: first string
 * @s2: second string
 * @i: index for s1
 * @j: index for s2
...
int compare_strings(char *s1, char *s2, int i, int j)
{
	...
	if (s1[i] == s2[j])
		return (compare_strings(s1, s2, i + 1, j + 1));
	else if (s2[j - 1] == '*' && s1[i] != '\0')
